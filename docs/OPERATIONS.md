...
| **Blast Radius Analyzer**| `g8s analyze --file <path> [--symbol <s>]` | Compute change risk score and suggested write receipt paths. |
| **Knowledge Vault Query**| `g8s vault query <search-term> [--limit 10]` | Perform BM25 ranked full-text search over Tri-Anchor records. |
| **Service Install** | `g8s service install` | Register background daemon (`launchd` on macOS, `systemd` on Linux). |
