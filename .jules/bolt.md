...
## 2025-02-18 - Avoid strings.ToLower for Prefix Checks in Loops
**Learning:** Using `strings.ToLower()` followed by `strings.HasPrefix()` or `strings.HasSuffix()` in tight loops (like process scanning) causes unnecessary heap allocations on every iteration.
**Action:** Use a length guard and `strings.EqualFold()` on a string slice (e.g., `len(s) >= 3 && strings.EqualFold(s[:3], "foo")`) for a zero-allocation case-insensitive comparison.
