...
func (d *DefaultProcessManager) FindGhostProcesses(ctx context.Context, heartbeatDir string, maxAge time.Duration, clock func() time.Time) ([]ProcessInfo, error) {
	...
	for _, p := range procs {
		...
		// Bolt Optimization: Replace strings.ToLower + HasPrefix with length check + EqualFold
		// to avoid allocating a new lowercased string for every running process on the system.
