...
func (d *DefaultProcessManager) FindGhostProcesses(ctx context.Context, heartbeatDir string, maxAge time.Duration, clock func() time.Time) ([]ProcessInfo, error) {
	...
	for _, p := range procs {
		...
		// ⚡ Bolt: Use length guards + strings.EqualFold on slices for zero-allocation case-insensitive prefix checks
		// This avoids strings.ToLower allocations inside the process loop
