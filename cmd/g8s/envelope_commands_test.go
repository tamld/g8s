...
func TestUnifiedEnvelopeCommands(t *testing.T) {
	...
	t.Run("submit, get, tasks, cancel, lineage lifecycle", func(t *testing.T) {
		// Submit
		env, code, raw := runCmd("submit", "--idempotency-key", "test-key-1", "--prompt", "Implement unified envelope", "--role", "collector", "--permission", "read_only", "--model", "gemini-3.8-flash-high", "--actor", "lead-dev")
		if code != 0 {
