...
func TestPrintUsageMentionsEveryLiveCommand(t *testing.T) {
	...
	for _, cmd := range []string{"submit", "get", "resume", "tasks", "lineage", "children", "receipt", "doctor", "init", "config", "completion", "service", "analyze", "vault", "worker", "mcp", "roles", "permissions", "version", "brief-issue", "brief-consume", "brief-list", "cleanup-worktrees", "self-audit"} {
