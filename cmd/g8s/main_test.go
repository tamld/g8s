...
func TestSubmitAndWorkerE2E(t *testing.T) {
  ...
  "providers": [
	...
	// Submit second task
	submitCmd2 := exec.Command(binPath, "submit", "--idempotency-key", "e2e-task-2", "--prompt", "failing prompt", "--model", "gemini-3.8-flash-high", "--json")
...
func TestNoColorInPipe(t *testing.T) {
	...
	submitCmd := exec.Command(binPath, "submit", "--idempotency-key", "no-color-task", "--prompt", "test ansi", "--model", "gemini-3.8-flash-high", "--json")
	submitCmd.Env = envVars
	submitOut, err := submitCmd.CombinedOutput()
	if err != nil {
		t.Fatalf("submit failed: %v\nOutput: %s", err, string(submitOut))
...
func TestSubmitPromptFromFile(t *testing.T) {
	...
	submitCmd := exec.Command(binPath, "submit", "--idempotency-key", "file-prompt-task", "--prompt-file", promptFile, "--model", "gemini-3.8-flash-high", "--json")
	submitCmd.Env = envVars
	submitOut, err := submitCmd.CombinedOutput()
	if err != nil {
...
func TestSubmitPromptFromStdin(t *testing.T) {
	...
	submitCmd := exec.Command(binPath, "submit", "--idempotency-key", "stdin-prompt-task", "--model", "gemini-3.8-flash-high", "--json")
	submitCmd.Env = envVars
...
func TestSubmitPromptFileError(t *testing.T) {
	...
	submitCmd := exec.Command(binPath, "submit", "--idempotency-key", "file-prompt-task", "--prompt-file", "/nonexistent/path/to/prompt.txt", "--model", "gemini-3.8-flash-high", "--json")
	submitCmd.Env = envVars
