...
func runCleanupWorktrees(args []string) {
	...
	failIf(err)
	if olderThan < 0 {
		failIf(errors.New("--older-than duration must be non-negative"))
	}

	report, err := executeCleanupWorktrees(os.Stdout, &DefaultGitRunner{}, time.Now, *repoDir, olderThan, *dryRun)
	failIf(err)
